...
use AppBundle\Form\Type\Security\RetrievePasswordType;
...
class DefaultController extends Controller
{
    ...
    public function passwordRetrievalAction(Request $request)
    {
        ...
        if ("POST" == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                $user = $this->getDoctrine()->getManager()->getRepository("AppBundle:User")->findOneBy(["username" => $user->getUsername()]);
        ...
        return $this->render("@App/Security/password-retrieval.html.twig", [
    ...
    public function resetPasswordAction(Request $request)
    {
        ...
        $user = $this->getDoctrine()->getManager()->getRepository("AppBundle:User")->findOneBy(["username" => $request->query->get("mail"), "passwordToken" => $request->query->get("token")]);
        ...
        if ("POST" == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                $existingUser = $this->getDoctrine()->getManager()->getRepository("AppBundle:User")->findOneBy(["username" => $user->getUsername(), "passwordToken" => $user->getPasswordToken()]);
        ...
        return $this->render("@App/Security/password-choose.html.twig", [
            "error" => $error,
            "form" => $form->createView(),
        ]);
    }
